import re

class MarathiPreprocessor:
    def __init__(self):
        # Basic Marathi stopwords
        self.stop_words = set(['आहे', 'आहेत', 'होता', 'होती', 'होते', 'आणि', 'किंवा', 'तर', 'मधे', 'वर', 'खाली', 'हे', 'ह्या', 'ह्याला', 'ह्यानी', 'त्या', 'त्याला', 'त्यानी'])

    def preprocess(self, text: str) -> dict:
        clean_text = re.sub(r'[^\u0900-\u097F\s]', '', text)
        clean_text = re.sub(r'\s+', ' ', clean_text).strip()
        tokens = clean_text.split()
        clean_tokens = [t for t in tokens if t not in self.stop_words]
        return {
            "original_text": text,
            "tokens": tokens,
            "clean_tokens": clean_tokens,
            "stemmed": clean_tokens,
            "lemmatized": clean_tokens
        }

if __name__ == "__main__":
    preprocessor = MarathiPreprocessor()
    print(preprocessor.preprocess("प्रकृती खूप छान आहे आज."))
